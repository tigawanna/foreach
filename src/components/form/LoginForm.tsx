...
const mutation = useMutation({
    mutationFn:({email,password}:LoginFormState)=>loginUser({email,password})
})    
...
return (
    <div className={cn("h-full w-full flex flex-col items-center justify-center gap-2")} {...props}>
        <form 
        onSubmit={handleSubmit} 
        className="w-[90%] md:w-[60%] p-5 rounded-2xl"
        ...
        >
            <div className="w-full flex flex-col items-center justify-center gap-2">
                ...
                <Button disabled={mutation.isPending} type="submit" 
                 className="shadow-secondary-foreground shadow hover:brightness-125 text-lg">
                    ...
                    {mutation.isError&&(
                        <p className="text-red-800  p-5">{mutation.error.message}</p>
                    )}
        ...
        variant="outline"  type="button" disabled={mutation.isPending} >
            {mutation.isPending ? (
                <Loader className="mr-2 h-4 w-4 animate-spin" />
            ) : (
